...
> **前置条件**：csc 源码仓库为私有仓库，需要在 `Settings → Secrets and variables → Actions` 中配置以下 Secrets：
>
> - `CSC_PAT`：对 `zgsm-sangfor/csc` 有 read 权限的 Personal Access Token，用于工作流拉取代码
> - `SSH_PRIVATE_KEY`：有权限访问 `zgsm-ai/costrict-review` 的 SSH 私钥（对应公钥需配置为 Deploy Key 或用户密钥），用于构建阶段下载 Builtin Review Skills
